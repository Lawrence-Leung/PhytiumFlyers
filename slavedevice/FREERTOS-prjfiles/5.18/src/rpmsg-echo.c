...
extern ZebraLine *zebraLineList;
extern TrafficLight *trafficLightList;
extern Stairs *stairsList;

extern unsigned char ObjectNum;
extern unsigned char ZebraLineNum;
extern unsigned char TrafficLightNum;
extern unsigned char StairsNum;
...
BaseType_t OpenAmpTask(void)
{
    ...
    //     (uint16_t)(1024),
    //     (void *)NULL,
    //     (UBaseType_t)configMAX_PRIORITIES - 6,    //相比之下设置更高的优先级
    //     NULL);
    // if (ret != pdPASS) {
    ...
    ret = xTaskCreate((TaskFunction_t)DHT11CircularRead,  /* task entry */
                      ...
                      (UBaseType_t)configMAX_PRIORITIES - 5,  /* task priority */
                      NULL); /* task handler */
	if (ret != pdPASS) {
