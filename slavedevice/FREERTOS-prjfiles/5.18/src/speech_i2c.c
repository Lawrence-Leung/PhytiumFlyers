...
void SpeechInit(void)
{
	IIC_Init();					//初始化
	SetVolume(1);				//设置音量
	SetReader(Reader_XiaoPing);	//设置人声
}
...
u8 GetChipStatus()
{
	...
	fsleep_millisec(100);
   	...
   	IIC_Start();          			//鍧姩鎬荤嚎
   	IIC_Send_Byte(I2C_ADDR+1);      //鍧戦�濆櫒浠跺湴鍧�
	if(ackflag==0)
	{
		return 0;
	}
	
	fsleep_millisec(5);//蹇呴』鍔犱竴鐐瑰欢鏃?
   	dat=IIC_Read_Byte();          //璇诲潠鏁版澁

   	IIC_Ack(1);           //鍧戦�濋潪搴旂瓟淇″澐
   	IIC_Stop();           //缁撴潫鎬荤嚎
   	return(dat);  
}
