...
{
    int ret = 0;
    struct rpmsg_endpoint lept1, lept2, lept3, lept4;  //测试创建2个端点，全静态。
    shutdown_req = 0;
    ...
    ECHO_DEV_SLAVE_DEBUG_I("[SLAVE] Try to create rpmsg endpoint [2].\r\n");
    taskENTER_CRITICAL();
    ret = rpmsg_create_ept(&lept3, rpdev, "syn6288", 30, RPMSG_ADDR_ANY, rpmsgSYN6288Response, rpmsg_service_unbind);
    taskEXIT_CRITICAL();
    if (ret)
    {
        ECHO_DEV_SLAVE_DEBUG_E("Failed to create endpoint. %d \r\n", ret);
    ...
    rpmsg_destroy_ept(&lept4);   //如果需要启用GPS传感器，取消注释掉这个就好
    taskEXIT_CRITICAL();
