        ...
        size_t start_block, size_t block_count)
{
    ...
    if (SDMMC_PTR_DMA_CAP(src)){
        ...
        if (err != SDMMC_OK) {
            SDMMC_LOGE(TAG, "%s: error 0x%x writing block %d+%d",
        ...
        size_t start_block, size_t block_count)
{
    ...
    if (host_is_spi(card)) {
        ...
        if (status & SD_SPI_R2_CARD_LOCKED) {
            SDMMC_LOGE(TAG, "%s: write failed, card is locked: r2=0x%04x",
                     __func__, status);
            return SDMMC_ERR_INVALID_STATE;
        }
        if (status != 0) {
            SDMMC_LOGE(TAG, "%s: card status indicates an error after write operation: r2=0x%04x",
                     __func__, status);
            return SDMMC_ERR_INVALID_RESPONSE;
        }
    }
        ...
        size_t start_block, size_t block_count)
{
    ...
    while (!host_is_spi(card) && !(status & MMC_R1_READY_FOR_DATA) && 
        ...
        if (++count % 10 == 0) {
            SDMMC_LOGD(TAG, "waiting for card to become ready (%d)", count);
        }
        sdmmc_sys_delay_ms(1U);
    }

    SDMMC_LOGD(TAG, "last card status = %d", SD_R1_CURRENT_STATE(status));
    return SDMMC_OK;
}
        ...
        size_t sector_count, sdmmc_erase_arg_t arg)
{
    ...
    sdmmc_err_t err = sdmmc_send_cmd(card, &cmd);
    if (err != SDMMC_OK) {
        SDMMC_LOGE(TAG, "%s: sdmmc_send_cmd (ERASE_GROUP_START) returned 0x%x", __func__, err);
        return err;
    }
    ...
    err = sdmmc_send_cmd(card, &cmd);
    if (err != SDMMC_OK) {
        SDMMC_LOGE(TAG, "%s: sdmmc_send_cmd (ERASE_GROUP_END) returned 0x%x", __func__, err);
        return err;
    }
    ...
    err = sdmmc_send_cmd(card, &cmd);
    if (err != SDMMC_OK) {
        SDMMC_LOGE(TAG, "%s: sdmmc_send_cmd (ERASE) returned 0x%x", __func__, err);
        return err;
    }
    ...
    if (host_is_spi(card)) {
        ...
        if (status != 0) {
            SDMMC_LOGE(TAG, "%s: card status indicates an error after erase operation: r2=0x%04x",
                     __func__, status);
            return SDMMC_ERR_INVALID_RESPONSE;
        }
    }
