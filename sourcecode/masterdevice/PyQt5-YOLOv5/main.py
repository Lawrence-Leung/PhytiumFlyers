from PySide6.QtWidgets import QApplication, QMainWindow, QFileDialog, QMenu, QWidget, QLabel
...
class MainWindow(QMainWindow, Ui_mainWindow):
    ...
    def run_or_continue(self):
        if self.runButton.isChecked():
            ...
            self.rtspButton.setEnabled(False)
            self.comboBox.setEnabled(False)
            self.checkBox.setEnabled(False)
            self.confSlider.setEnabled(False)
            self.confSpinBox.setEnabled(False)
            self.iouSpinBox.setEnabled(False)
            self.iouSlider.setEnabled(False)
            self.rateSpinBox.setEnabled(False)
    ...
    def show_statistic(self, statistic):
        msg = ''
    ...
    def max_or_restore(self):
        if self.m_flag:
            ...
            self.showMaximized()
            self.m_flag = True
            self.maxButton.setText('O')

    def show_image(self, img, label):
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        if label == self.raw_video:
