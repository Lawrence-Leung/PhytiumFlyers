...
import numpy as np
import random
from PySide6.QtCore import QObject, Signal,QPoint ,Qt,QRect
from PySide6.QtGui import QPainter,QPolygonF, QColor,QImage


#信号中介类
class MapUpdateNotifier(QObject):
    background_changed = Signal(np.ndarray)
...
class LittleMap(QObject):
    def __init__(self):
        ...
        self.objects = []
        self.map_update_notifier = MapUpdateNotifier()
    # 一共mapnumcircle条环状线，每条的距离为2.5m，总共可以判定盲人周边10米以内的距离。

    # 初始化并清空图像。这个图像是320*320大小，由辐射状和环状网格组成，背景纯白，就像雷达一样
    def clearMap(self,objects):
        print ("clearMap")
        # 初始化背景
        
        self.objects =objects
        self.qimage = QImage(self.background.data, self.background.shape[1], self.background.shape[0], QImage.Format_RGB888)
        painter = QPainter(self.qimage)
        painter.setPen(Qt.black)
        painter.eraseRect(0, 0, self.mapsize, self.mapsize)#清除画布上之前的绘制痕迹，解决叠涂
        for i in range(1, self.mapnumcircle + 1):
            radius = i * (self.mapsize // 2) // self.mapnumcircle
            painter.drawEllipse(self.mapcenter[0] - radius, self.mapcenter[1] - radius, 2 * radius, 2 * radius)

        for i in range(self.mapnumlines):
            angle = 2 * 3.1415926535 * i / self.mapnumlines
            x = self.mapcenter[0] + (self.mapsize // 2) * np.cos(angle)
            y = self.mapcenter[1] + (self.mapsize // 2) * np.sin(angle)
            painter.drawLine(self.mapcenter[0], self.mapcenter[1], x, y)
        self.drawMyCenterPosition(painter)
        # self.showObjects(painter)
        painter.end()
        return self.qimage
    ...
    def drawMyCenterPosition(self,painter,angle_offset=0):
        # clear_rect = QRect(0, 0, self.mapsize, self.mapsize)
        # 定义三角形的三个顶点
        triangle_size = 10
        triangle_points = [
            QPoint(self.mapcenter[0], self.mapcenter[1] - triangle_size),
            QPoint(self.mapcenter[0] - triangle_size * np.sin(np.pi / 3), self.mapcenter[1] + triangle_size / 2),
            QPoint(self.mapcenter[0] + triangle_size * np.sin(np.pi / 3), self.mapcenter[1] + triangle_size / 2)
        ]
        painter.setBrush(Qt.blue)
        painter.drawPolygon(QPolygonF(triangle_points))
        self.max_distance = self.circledistance * self.mapnumcircle
        print("object_num:",len(self.objects) )
        ...
        for obj in self.objects:
            ...
            x = self.mapcenter[0] + (distance / self.max_distance) * (self.mapsize / 2) * np.cos(angle_rad)
            y = self.mapcenter[1] - (distance / self.max_distance) * (self.mapsize / 2) * np.sin(angle_rad)
            color = QColor(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
            painter.setBrush(color)
            painter.drawEllipse(x - 3, y - 3, 6, 6)
            painter.drawText(x + 5, y - 5, detection_result)
    ...
    #     self.background_changed.emit(self.background)
    def get_background_image(self):
        self.showObjects()
        return self.background        
    def emit_background_changed_signal(self,objects):
        # background_img = self.get_background_image()
        self.map_update_notifier.background_changed.emit(objects)
