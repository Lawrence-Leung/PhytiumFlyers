...
class RPMsg:
    ...
    def writeEptDevice(self):
        ...
        if self.eptfd<0:
            print("[OPENAMP] Invalid file descriptor")
            return
        
        # string = "I'm Lawrence '%s" % self.ept_device_path
        string="未摔倒"
        self.buf0 = string.encode('gbk')   # 为字符串编码为ASCII
        self.buf0 = self.buf0.ljust(32, b'\x00')  # 确保不超过32字节

        #print("buf0:",self.buf0)
        # 开始写入数据
        self.ret = os.write(self.eptfd, self.buf0)
        if self.ret < 0:
            print("[OPENAMP] Write endpoint '%s' failed" % self.ept_device_path)
    
    def writeSpeaker(self,string):
        # todo: demo only，后续需要加入业务代码，如JSON之类的
        if self.eptfd<0:
            print("[OPENAMP] Invalid file descriptor")
            return
