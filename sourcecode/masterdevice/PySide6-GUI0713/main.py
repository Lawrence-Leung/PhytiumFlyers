...
from slaveTest import SlaveTest
from slaveThread import SlaveDeviceThread #slaveThread/slaveThread1
...
################################################################以上内容每次移植都不作修改,以下设置线程/进程类初始化以及槽函数连接
        ...
        #self.slave_thread.speak.connect(self.video_detection_system.handle_ept_device_callback)
        self.video_detection_system.broadcast_signal.connect(self.slave_thread.handle_broadcast)

        ################################################################
        # 以下代码是pc端slave和master通信的测试代码
    ...
    def update_map(self, image):
        #print("update_map is running in thread:", threading.current_thread().ident)
        label_width = self.pre_video.width()
        label_height = self.pre_video.height()
        scaled_img = image.scaled(label_width, label_height)
