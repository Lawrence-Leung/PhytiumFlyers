...
class SlaveDeviceThread(QObject):
    ...
    def timetosend(self):
        ...
        self.timerS.start(1)  # 每毫秒发射一次传感器信号到ui
        self.timerR = QTimer()# 这个定时器控制发播报到RPMSG的时间间隔
        self.timerR.timeout.connect(self.send_broadcast)
        self.timerR.start(6500)#间隔6.5s
    ...
    def handle_broadcast(self,content):
        ...
        while not self.speak_queue.empty():
            try:
                        ...
                        print("speak_queue is empty")
                        break        
        self.speak_queue.put(content)
           
        
    def send_broadcast(self):
        if not self.speak_queue.empty():
            content=self.speak_queue.get()
            print("发送播报：",content)
            ...
            print(f'[播报1]: {SpeakObstacles(self.speaker.objects)}')  
            zebrastring, trafficstring = SpeakZebraTraffic(self.speaker.zebralines, self.speaker.traffic_lights) 
            print(f'[播报2,3]: {zebrastring} {trafficstring}')
            print(f'[播报4]" {SpeakStairs(self.speaker.stairs)}') 
