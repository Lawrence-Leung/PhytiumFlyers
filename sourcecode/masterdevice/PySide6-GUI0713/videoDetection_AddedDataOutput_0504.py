...
class VideoDetectionSystem(QObject):
    ...
    broadcast_signal = Signal(bytearray)
    def __init__(self):
        super().__init__()
    ...
    def capture_and_detect_image(self,image_queue,
        ...
        while cap.isOpened():
            ...
            if ret: # 成功读取到信息
                ...
                buf0 = array_string.encode('ascii')   # 为字符串编码为ASCII
                print("buf0:",buf0)
                buf0 = buf0.ljust(32, b'\x00')  # 确保不超过32字节
                print("buf00:",buf0)
                self.broadcast_signal.emit(array)
                if isinstance(array, bytearray) and len(array) > 0:
                    ...
                    # 将GPS数据放到这里来
                    self.current_location = (113.32405139420317, 23.096203484758096) # todo：这里要把它替换成由GPS传感器传过来的
                    if self.current_location is not None:
                        isnearroad, isnearcrossing, isalongroad, nearest_road = deviation_detector.CompleteRoadDeviationProcess(test_road_lists, self.current_location)  # 分析行人是否沿着道路行走
...
class MainWindow(Ui_MainWindow, QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        UIFuncitons.shadow_style(self, self.Class_QF_2, QColor(94,96,92))
        UIFuncitons.shadow_style(self, self.Fps_QF_2, QColor(211,215,207))
        UIFuncitons.shadow_style(self, self.Model_QF_2, QColor(136,138,133))
        #左菜单栏
        ...
        self.run_button.clicked.connect(self.source_using)
