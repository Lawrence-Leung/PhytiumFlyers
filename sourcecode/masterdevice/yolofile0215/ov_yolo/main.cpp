...
string ir_filename = "/home/jimkwokying/projectTest/yolov5-openvino-cpp-python/model/yolov5n.xml"; 
// /home/jimkwokying/projectTest/yolov5-openvino-cpp-python/model/yolov5n.xml 
//  /home/jimkwokying/ir/yolov5n.xml
...
void fill_tensor_data_image(ov::Tensor& input_tensor, const cv::Mat& input_image) {
	...
	const size_t width = 640; // 要求输入图片数据的宽度
	const size_t height = 640; // 要求输入图片数据的高度
...
int main(int argc, char** argv) {
	...
	//1、创建OpenVINO Core
	Core core;
	CompiledModel compiled_model = core.compile_model(ir_filename, "CPU");
	std::cout << "run on CPU" << std::endl;
	InferRequest infer_request = compiled_model.create_infer_request();


	//2、 预处理输入数据 - 格式化操作
	VideoCapture cap(("/home/jimkwokying/Videos/sample.mp4"));
	// cap.open(0);
	...
	int input_h = 640; //获得"images"节点的Height
	int input_w = 640; //获得"images"节点的Width
	...
	while (true) {
		...
		// 6、解析推理结果，YOLOv5 output format: cx,cy,w,h,score
