...
{			
    // Jy61pSclInitOut();
    // Jy61pSdaInitOut();
    GpioInit(JY61P_SDA_INDEX,&jy61p_sda_gpio, jy61p_sda_cfg, JY61P_SDA_config);
	GpioInit(JY61P_SCL_INDEX,&jy61p_scl_gpio, jy61p_scl_cfg, JY61P_SCL_config);
	
	SDA_OUT(jy61p_sda_gpio,JY61P_SDA_config); 
	// IIC_SDA=1;	  	  
	// IIC_SCL=1;
    JY61P_SDA_HIGH;
    JY61P_SCL_HIGH;
...
u8 IIC_Wait_Ack(void)
{
	u8 ucErrTime=0; 
	// SDA_IN();
    SDA_OUT(jy61p_sda_gpio,JY61P_SDA_config);               //先设置为输出
	JY61P_SDA_HIGH;
		// Delay(5);	
    fsleep_microsec(13);  
    SDA_IN(jy61p_sda_gpio,JY61P_SDA_config);               //将上面的输入移到这里
	while(READ_SDA)
	{
		ucErrTime++;
		if(ucErrTime>50)
...
u8 IIC_Read_Byte(unsigned char ack)
{
	...
	SDA_IN(jy61p_sda_gpio,JY61P_SDA_config);
    for(i=0;i<8;i++ )
	{
        JY61P_SCL_LOW; 
