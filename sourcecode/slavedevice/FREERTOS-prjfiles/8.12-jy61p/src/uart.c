...
static u8 recv_buffer_i[USART_REC_LEN]; /*intr Buffer for Receiving Data */
// char rx[10];
u16 cnt = 0;
_SaveData Save_Data;
...
void UartWaitLoop(void)
{
    ...
    {
        taskENTER_CRITICAL();
        err = FtFreertosUartReceiveBuffer(&uart, recv_buffer_i, sizeof(recv_buffer_i), &recive_length);
        if (err != FT_SUCCESS)
            printf("[GPS]error!");
