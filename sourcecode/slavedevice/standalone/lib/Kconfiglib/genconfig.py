...
def mk_sdkconfig(filename):
    ...
    sdkconfig = open('sdkconfig.h', 'w')
    sdkconfig.write('#ifndef SDK_CONFIG_H__\n')
    ...
    for line in config:
        ...
        if line[0] == '#':
            ...
            if len(setting) >= 2:
                ...
                if setting[1] == 'y':
                    sdkconfig.write('#define %s\n' % setting[0])
                else:
                    sdkconfig.write('#define %s %s\n' % (setting[0], re.findall(r"^.*?=(.*)$",line)[0]))

    if os.path.isfile('sdkconfig_project.h'):
        sdkconfig.write('#include "sdkconfig_project.h"\n')
...
""")
    ...
    if args.header_path is None:
        if "KCONFIG_AUTOHEADER" in os.environ:
            ...
            # as well.
            kconf.write_autoconf("sdkconfig.h")
    else:
