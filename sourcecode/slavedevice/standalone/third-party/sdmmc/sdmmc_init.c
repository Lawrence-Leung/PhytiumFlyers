...
#define SDMMC_INIT_STEP(condition, function) \
    do { \
        if ((condition)) { \
            ...
            if (err != SDMMC_OK) { \
                SDMMC_LOGE(TAG, "%s: %s returned 0x%x", __func__, #function, err); \
                return err; \
            } else {\
                SDMMC_LOGI(TAG, "%s: %s success", __func__, #function);\
            }\
        } \
    } while(0);
...
sdmmc_err_t sdmmc_card_init(const sdmmc_host_t* config, sdmmc_card_t* card)
{
    ...
    memcpy(&card->host, config, sizeof(*config)); /* make a copy of host to card */
