...
static inline int usbh_msc_scsi_testunitready(struct usbh_msc *msc_class)
{
    ...
    /* Construct the CBW */
    cbw = (struct CBW *)msc_class->g_msc_buf;
    ...
    return usbh_bulk_cbw_csw_xfer(msc_class, cbw, (struct CSW *)msc_class->g_msc_buf, NULL);
}
...
static inline int usbh_msc_scsi_requestsense(struct usbh_msc *msc_class)
{
    ...
    return usbh_bulk_cbw_csw_xfer(msc_class, cbw, (struct CSW *)msc_class->g_msc_buf, msc_class->g_msc_buf);
}
...
int usbh_msc_scsi_write10(struct usbh_msc *msc_class, uint32_t start_sector, const uint8_t *buffer, uint32_t nsectors)
{
    ...
    return usbh_bulk_cbw_csw_xfer(msc_class, cbw, (struct CSW *)msc_class->g_msc_buf, (uint8_t *)buffer);
}
...
static int usbh_msc_connect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    ret = usbh_msc_get_maxlun(msc_class, msc_class->g_msc_buf);
    if (ret < 0) {
        return ret;
    }

    USB_LOG_INFO("Get max LUN:%u\r\n", msc_class->g_msc_buf[0] + 1);
    ...
    snprintf(hport->config.intf[intf].devname, CONFIG_USBHOST_DEV_NAMELEN, DEV_FORMAT, usb->id, msc_class->sdchar);

    USB_LOG_RAW("Register MSC Class:%s for USB-%d\r\n", hport->config.intf[intf].devname, usb->id);
...
static int usbh_msc_disconnect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    if (msc_class) {
        ...
        if (hport->config.intf[intf].devname[0] != '\0')
            USB_LOG_RAW("Unregister MSC Class:%s\r\n", hport->config.intf[intf].devname);
    }
