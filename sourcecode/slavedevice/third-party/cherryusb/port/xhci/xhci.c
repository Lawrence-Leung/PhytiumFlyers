...
int xhci_probe ( struct xhci_host *xhci, unsigned long baseaddr ) {
	...
	for ( i = 1 ; i <= xhci->ports ; i++ ) {
		port = usbh_get_roothub_port ( xhci->bus, i );
...
static int xhci_scratchpad_alloc ( struct xhci_host *xhci ) {
	...
	for ( i = 0 ; i < scratch->count ; i++ ) {
		scratch->array[i] = CPU_TO_LE64 ( (uintptr_t)addr );
		addr += xhci->pagesize;
	}

	/* Set scratchpad array pointer */
	USB_ASSERT ( xhci->dcbaa.context != NULL );
	xhci->dcbaa.context[0] = CPU_TO_LE64 ( ( (uintptr_t)scratch->array ) );
...
static void xhci_trb_queue(struct xhci_ring *ring, union xhci_trb *trb) {
	...
	if (ring->nidx >= XHCI_RING_ITEMS - 1) {
		...
		link_trb.link.next = CPU_TO_LE64(((uintptr_t)ring->ring));
...
void xhci_endpoint_message ( struct xhci_endpoint *ep, 
	...
	if (datalen > 0) {
		...
		data->data = CPU_TO_LE64 ( (uintptr_t)data_buff );
...
void xhci_event_process(struct xhci_host *xhci) {
    ...
    for (;;) {
        ...
        trb = evts->ring + nidx; /* current trb */
        control = trb->common.flags; /* trb control field */
		...
		default:
			USB_LOG_DBG("XHCI %s unrecognised event type %d\n:",
			       xhci->name, ( evt_type ) );
			break;
        }
